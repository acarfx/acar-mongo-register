...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
	...
	message.channel.send(embed.setDescription(`${uye} kişisinin toplamda ${isimveri.Isimler.length || 0} isim kayıtı bulundu.\n${isimler}`).setFooter(ayarlar.embed.altbaşlık));
    } else {
         message.channel.send(embed.setDescription(`${uye} kişisinin isim kayıtı bulunamadı.`).setFooter(ayarlar.embed.altbaşlık));
     }
    }
};